...
const ChatCtl = {
    async ChatSocket(socket, request) {
        ...
        try {
            ...
            if (this.activeConnections.has(userId)) {
                this.activeConnections.get(userId).close(4009, "Replaced by new connection");
            }
            this.activeConnections.set(userId, socket);

            const unread = await ChatModel.chat_get_unread(this.db, userId);
            if (unread.success && unread.result.length > 0) {
                unread.result.forEach((msg) => socket.send(JSON.stringify(msg)));
            ...
            socket.on("message", async (rawMessage) => {
                try {
                    ...
                    const blockCheck = await FriendshipModel.check_block(this.db, userId, message.to);
                    if (blockCheck.result) return;

                    const recipientSocket = this.activeConnections.get(message.to);
    ...
    async ChatHistory(request, reply) {
        ...
        if (!rec_check.success) {
            return reply.code(404).send({ success: false, result: "Recipient not found" });
        }
    ...
    async ChatProfiles(request, reply) {
        const res = await ChatModel.chat_get_profiles(this.db, request.user.payload.id);
        reply.code(res.code).send(res);
    },
};
