...
const ActionButton = (
    ...
    return (
        <div className="flex gap-2">
            <Button
                variant="outline"
                onClick={() => router.push(`/chat?with=${targetUser.id}`)}
            >
...
export default function ProfilePage({ params }: ProfilePageProps) {
    ...
    const winLossRatio =
        (profile.loses > 0 ? (profile.wins / profile.loses) : profile.wins)
            .toFixed(2);
        ...
        <div className="container mx-auto max-w-7xl py-8 space-y-8 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard
                    ...
                    value={winLossRatio}
                    icon={<Ratio className="h-5 w-5 text-yellow-400" />}
                />
                <StatCard
                    title="Total Games"
                    value={totalGames}
                    ...
                    <CardContent className="space-y-3 max-h-96 overflow-y-auto">
                        {matchHistory.length > 0
                            ? matchHistory.map((match) => {
                                ...
                                const opponentName =
                                    match.player1_name === profile.name
                                        ? match.player2_name
                                        : match.player1_name;
                                return (
                                    <div
                                        key={match.id}
                                        className={`p-3 rounded-md flex justify-between items-center border-l-4 ${
                                            isWinner
