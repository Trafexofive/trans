...
const ActionButton = (
    ...
    const receivedRequest = requestStatuses.received.find((req: any) =>
        req.sender_id === targetUser.id
    );

    if (isFriend) {
        return (
            <button
                onClick={() => friendAction(targetUser.id, "remove")}
    ...
    if (sentRequest) {
        return (
            <button
                onClick={() =>
                    friendAction(targetUser.id, "cancel", sentRequest.id)}
    ...
    if (receivedRequest) {
        return (
            <button
                onClick={() =>
                    friendAction(targetUser.id, "accept", receivedRequest.id)}
    ...
    return (
        <button
            onClick={() => friendAction(targetUser.id, "invite")}
            className="btn btn-primary"
        >
            Add Friend
        </button>
...
export default function ProfilePage({ params }: ProfilePageProps) {
    ...
    }, [userId, fetchProfileData, fetchFriendData, loggedInUser]);
                        ...
                        <div className="settings-content grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="md:col-span-2">
                                ...
                                <div className="profile-stats">
                                    ...
                                    <StatCard
                                        title="Total Games"
                                        value={profile.wins + profile.loses}
                                    />
                                </div>
                                <div className="mt-8">
                                    <h3 className="section-title">
                                    ...
                                    {friends.length > 0
                                        ? friends.map((friend) => (
                                            <Link
                                                ...
                                                key={friend.id}
                                                className="flex items-center gap-3 p-2 rounded hover:bg-[#29282b] transition-colors"
                                            >
                                                <img
                                                    src={friend.avatar ||
                                                        "/avatars/default.png"}
