...
export default function ChatPage() {
    ...
    const fetchBlockStatus = useCallback(async (partnerId: number) => {
        ...
        try {
            const res = await fetch(
                `${API_BASE_URL}/api/friendships/block/${partnerId}`,
    ...
    const handleFriendAndBlockAction = async (
        action: "remove" | "block" | "unblock",
    ) => {
        if (!activePartner) return;
        let confirmText =
            `Are you sure you want to ${action} ${activePartner.name}?`;
        if (action === "block") {
            confirmText += " This will also remove them as a friend.";
        }
                        ...
                        <div className="conversation-list">
                            {chatPartners.map((partner) => (
                                <div
                                    key={partner.id}
                                    onClick={() =>
                                        router.push(`/chat?with=${partner.id}`)}
                                ...
                                >
                                    <div className="relative">
                                        ...
                                        {unreadFrom.has(partner.id) && (
                                            <span className="absolute top-0 left-0 block h-3 w-3 rounded-full bg-green-400 ring-2 ring-gray-800">
                                            </span>
                                        )}
                                    </div>
