...
import { Crown, Info, Loader2, Sword, Trophy, Users } from "lucide-react";
...
export default function TournamentDetailPage({ params }: TournamentPageProps) {
    ...
    const handleAction = async (action: "join" | "start") => {
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/api/tournaments/${tournamentId}/${action}`,
                {
                    method: "POST",
                    headers: { "Authorization": `Bearer ${accessToken}` },
                },
            ...
            const data = await response.json();
            if (!data.success) throw new Error(data.result);
            await fetchTournamentData(); // Fetch immediately after action
        } catch (err: any) {
            setError(err.message);
        } finally {
    ...
    const getTournamentWinner = () => {
        if (
            tournament?.details.status !== "completed" ||
            !tournament.matches.length
        ) return null;
        const finalRound = Math.max(
            ...tournament.matches.map((m) => m.round_number),
        );
                ...
                <div>
                    ...
                    <p className="text-muted-foreground">
                        ...
                        <span className="font-semibold capitalize">
                            {tournament.details.status.replace("_", " ")}
                        </span>
                    </p>
                </div>
                    ...
                    <div className="flex gap-2">
                        {!isUserInTournament && (
                            <Button
                                ...
                                disabled={isActionLoading || isUserInTournament}
                            >
                                Join Tournament
                            </Button>
                        )}
                    ...
                    <div className="flex gap-8 overflow-x-auto pb-4">
                        {Object.entries(rounds).length > 0
                            ? Object.entries(rounds).map((
                                ...
                                >
                                    <h3 className="text-xl font-bold text-center text-muted-foreground">
                                        Round {roundNumber}
                                    </h3>
                                    {matches.map((match) => (
                                        <Card
                                                ...
                                                {match.status === "pending" &&
                                                    ...
                                                    (
                                                        <Button
                                                            ...
                                                            onClick={() =>
                                                                router.push(
                                                                    `/play?matchId=${match.id}&tournamentId=${tournamentId}`,
                                                                )}
                                                        >
                            ...
                            : (
                                <div className="w-full text-center text-muted-foreground py-10 border-2 border-dashed rounded-lg">
                                    ...
                                    <p className="mt-1 text-sm">
                                        The bracket will be generated once the
                ...
                <Card className="lg:col-span-1 h-fit">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            <Users />Participants ({tournament.participants
                                .length})
                        </CardTitle>
                    </CardHeader>
