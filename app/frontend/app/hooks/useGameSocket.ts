...
export const useGameSocket = (accessToken: string | null, matchId: string | null) => {
  ...
  const playerInfoRef = useRef<{ p1: PlayerInfo | null, p2: PlayerInfo | null }>({ p1: null, p2: null });

  useEffect(() => {
    // --- THE FIX: Only check for the accessToken. ---
    // A null matchId is a valid state for public matchmaking.
    if (!accessToken) {
    ...
    socket.onmessage = (event) => {
      ...
      switch (message.type) {
        case 'gameStart':
          ...
          playerInfoRef.current = { p1: message.payload.player1, p2: message.payload.player2 };
          setWinner(null);
          break;
        case 'gameState':
